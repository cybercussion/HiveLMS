function TouchEventManager() {
    ...
    this.init = function() {
        ...
        this.useTouch = ('ontouchstart' in window) && !/chrome/i.test(this.userAgent);
    ...
    this.onTouchStart = function(e) {
        var me = portal.toucheventmanager,
    ...
    this.startTrack = function(touch, e) {
        var me = this,
        ...
        if (/android/i.test(this.userAgent)) {      
            if (!['input', 'select', 'textarea'].contains(target.tagName.toLowerCase())) {
            	debug("This is not a input field ", 4);
                e.preventDefault();
                e.stopPropagation();                
            } else {
            	debug("this is a input field", 4);
            }
        }
    ...
    this.addEventListener = function(dom, ename, fn, o) {
        ...
        for (id in this.tracks) {
            ...
            if (track && (dom == document || dom === track.target || $(dom).attr('id').contains(track.target))) {
                track.events[ename] = track.events[ename] || {};
    ...
    this.removeEventListener = function(dom, ename, fn) {
        if (!this.targets[ename]) {
            return;
        }
    ...
    this.fireListeners = function(ename, track, e) {
        var me = portal.toucheventmanager;
